...
class MetricsStore:
    ...
    def __init__(self, db_path: str | Path | None = None) -> None:
        ...
        # Reentrant: guarded access may be reached from inside another
        # guarded section, and a plain Lock would deadlock there.
        self._lock = threading.RLock()
        self._closed = False
        self._conn: sqlite3.Connection | None = None

    def _get_conn(self) -> sqlite3.Connection:
        """Return the connection, opening it on first use.

        The lock is held while opening so two threads cannot each build a
        connection and leave one orphaned, and refuses use after close so a
        race with close() raises a clear error rather than a bare
        sqlite3.ProgrammingError from deep inside a query.
        """
        with self._lock:
            if self._closed:
                raise RuntimeError("MetricsStore is closed")
    ...
    def close(self) -> None:
        ...
        Idempotent, and holds the lock so it cannot run while another thread
        is opening or using the connection.
        """
        with self._lock:
            self._closed = True
            if self._conn:
            ...
            for point in data_points:
                ...
                try:
                    cursor = conn.execute(
                        """
                        INSERT OR IGNORE INTO client_metrics
                            (client_uuid, client_name, metric, value,
                             recorded_at, synced_at, source)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                        """,
